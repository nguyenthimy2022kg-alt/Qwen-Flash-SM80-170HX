...
The reference machine uses an additional CMP driver overlay. This repository now provides a [BAR1/P2P reference build](../drivers/cmp-bar1/README.en.md), not a complete copy of the running driver. See the [CMP P2P deployment reference](CMP_P2P.en.md) for its differences from public cmpunlocker. Machines that meet the [host acceptance criteria](DEPLOYMENT.en.md#host-acceptance-criteria) through native support or another implementation do not need the same patches.
