...
**实现边界：** 基础 cmpunlocker 还包含 PCIe Gen2 适配，三个增量补丁不能独立运行。0011 除互传映射外，还修改全局 ReBAR 默认值、放宽部分映射销毁诊断，并包含 Blackwell 分支变化；不能把它视为完全限定于 CMP 的修改。0015 不检测实际 PCIe 路由，不能保证其他硬件能直传。准备脚本只写源码目录与缓存，不启用本机的显存超频/时序配置。
