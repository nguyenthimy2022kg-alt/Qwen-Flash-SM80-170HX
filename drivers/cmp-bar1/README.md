...
**状态：这是基于公开固定版本源码整理的参考构建，并非当前运行驱动的逐字复制。**
本机历史驱动的双卡互传和 NVMe 直读已经验证；此整理版的编译检查不能代替装机后的运行验证。

原理与选择：先看 [各组件分工、BAR1 扩容与映射、三个补丁的作用](../../docs/CMP_P2P.md)。本页接着说明具体构建与安装。

## 包含什么

| 部分 | 解决的问题 / 提供的功能 |
|---|---|
| 固定 cmpunlocker 基础 | CMP 初始化、显存容量解锁、BAR1 扩容和 P2P 支持，是增量补丁的基础 |
| `0011`：接通映射 | 建立 BAR1 互传所需的地址映射和页表处理 |
| `0013`：排除冲突 | 使用 BAR1 时跳过 mailbox（另一种互传机制）的预登记，避免互相阻塞 |
| `0015`：放开平台判断，可选 | 对指定 CMP 型号将平台读取能力改为“允许”；需要显式选择，并在目标机器验证真实互传 |
| `prepare.py`：准备构建 | 下载并校验固定源码，生成配置并应用补丁；下一步才是编译和手动安装 |

**实现边界：** 基础 cmpunlocker 还包含 PCIe Gen2 适配，三个增量补丁不能独立运行。0011 除互传映射外，还修改全局 ReBAR 默认值、放宽部分映射销毁诊断，并包含 Blackwell 分支变化；不能把它视为完全限定于 CMP 的修改。0015 不检测实际 PCIe 路由，不能保证其他硬件能直传。准备脚本只写源码目录与缓存。
