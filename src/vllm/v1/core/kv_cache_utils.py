...
def _get_kv_cache_groups_csa_linear(
    ...
    for tp_replicated in (False, True):
        ...
        # PP ranks can expose distinct logical state shapes even when every
        # cache owner has the same physical page and scheduling geometry.
        # Those fields, plus the copy-function dispatch type, are the actual
        # invariants required by the shared CSA tensor group.
        def _compatible_mamba_spec(spec):
            return (
                spec.block_size == representative.block_size
                and spec.page_size_bytes == representative.page_size_bytes
                ...
                and spec.mamba_type == representative.mamba_type
                and spec.tp_replicated == representative.tp_replicated
            )

        if any(not _compatible_mamba_spec(roles.mamba[name]) for name in names[1:]):
            policy = "replicated" if tp_replicated else "sharded"
            details = {name: repr(roles.mamba[name]) for name in names}
            raise ValueError(
                f"CSA+linear {policy} mamba cache owners have incompatible "
                f"physical/scheduling specs: {details}"
            )
