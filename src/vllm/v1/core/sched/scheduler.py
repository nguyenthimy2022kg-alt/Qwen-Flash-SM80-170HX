...
class Scheduler(SchedulerInterface):
    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            # With V2 pipeline scheduling, the prior bonus token can still be an
            # in-flight output placeholder when the next MTP verification step is
            # formed. The worker nevertheless needs one query row for that bonus
            # plus one row per draft token. Preserve that existing worker invariant
            # by making the verification batch at least draft + bonus wide.
            min_verification_tokens = 0
            if request.spec_token_ids and not request.is_prefill_chunk:
                min_verification_tokens = (
                    len(request.spec_token_ids) + self.num_sampled_tokens_per_step
                )
                num_new_tokens = max(num_new_tokens, min_verification_tokens)
            ...
            # Never send a partial verification batch after token, input,
            # model-length, Mamba, encoder, or prefill-lookahead caps are applied.
            if 0 < num_new_tokens < min_verification_tokens:
                num_new_tokens = 0
