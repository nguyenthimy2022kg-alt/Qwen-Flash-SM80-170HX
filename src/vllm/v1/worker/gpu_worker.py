...
class Worker(WorkerBase):
    ...
    def _has_ple_layers(self) -> bool:
        """Return whether this PP rank owns one of the model's PLE layers."""
        if not envs.VLLM_PLE_CPU_OFFLOAD:
            return False
        text_config = self.model_config.hf_text_config
        ple_layer_ids = tuple(int(i) - 1 for i in text_config.ple_layer_ids)
        ...
        return any(start <= layer_id < end for layer_id in ple_layer_ids)
    ...
    def _validate_ple_offload_config(self) -> None:
        ...
        if parallel_config.pipeline_parallel_size != 1:
            # Async PLE is safe with PP when every PLE layer is owned by this
            # single stage. Only that stage registers with the CPU worker.
            from vllm.distributed.utils import get_pp_indices
            ...
            ple_layers = tuple(int(i) - 1 for i in text_config.ple_layer_ids)
            if not ple_layers or not all(start <= i < end for i in ple_layers):
                unsupported.append(
                    f"PP={parallel_config.pipeline_parallel_size} with PLE split "
    ...
    def compile_or_warm_up_model(self) -> CompilationTimes:
        ...
        if (
            ...
            # The first non-last PP rank updates this counter after sampling.
            # Loading its tiny Triton module while a PP send/recv is outstanding
            # can stall the CUDA driver and leave the next rank blocked in recv.
            # Load it locally before any pipeline communication is in flight.
            from vllm.v1.worker.gpu.input_batch import (
                post_update_num_computed_tokens,
            )

            _idx = torch.zeros(1, dtype=torch.int32, device=self.device)
            _count = torch.zeros(1, dtype=torch.int32, device=self.device)
            _qsl = torch.tensor([0, 1], dtype=torch.int32, device=self.device)
            post_update_num_computed_tokens(_idx, _count, _qsl)
            torch.accelerator.synchronize()
            logger.info(
                "Preloaded PP post-update Triton kernel before pipeline traffic."
            )
        ...
        if self.use_v2_model_runner:
            # The synthetic V2 warmup has no scheduler to drive the PP pipeline.
            # With a last-rank-only MTP speculator it deadlocks in the first
            # irecv after graph capture. Real requests use the async scheduler
            # and warm these residual JIT paths safely on first use.
            if (
                self.parallel_config.pipeline_parallel_size > 1
                and self.speculative_config is not None
            ):
                logger.warning(
                    "Skipping synthetic V2 kernel warmup for PP+speculative "
                    "decoding; real async-scheduled requests will warm it."
                )
            else:
                warmup_kernels(
        ...
        # PP+speculative V2 intentionally skips the scheduler-less synthetic
        # warmup above. Its first real request therefore still compiles a few
        # small Triton kernels. Triton's post-compile observability hook enters
        # launch-metadata handle initialization while the next PP rank is
        # blocked in recv, which can stall indefinitely on this SM80 setup.
        # The monitor is diagnostic only, so omit it for this exact layout.
        if not (
            self.use_v2_model_runner
            and self.parallel_config.pipeline_parallel_size > 1
            and self.speculative_config is not None
            ...
            logger.warning(
                "Disabling the diagnostic JIT monitor for PP+speculative V2 "
                "to avoid a first-request Triton hook deadlock."
            )
