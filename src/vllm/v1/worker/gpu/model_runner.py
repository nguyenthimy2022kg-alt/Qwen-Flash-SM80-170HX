...
from vllm.model_executor.layers.mamba.gdn import gdn_graph_runtime
...
def _r38_rank_local_graph_mode(
    configured_mode: CUDAGraphMode, pp_rank: int, pp_size: int
) -> CUDAGraphMode:
    """Diagnostic-only PP0 PIECEWISE / PP1 FULL decode selection."""
    if os.getenv("R38_PP1_FULL_DECODE", "0") != "1":
        return configured_mode
    if pp_size != 2:
        raise RuntimeError(f"R38 rank-local graph mode requires PP2, got PP{pp_size}")
    if configured_mode != CUDAGraphMode.FULL_AND_PIECEWISE:
        raise RuntimeError(
            "R38 rank-local graph mode requires configured FULL_AND_PIECEWISE, "
            f"got {configured_mode.name}"
        )
    return (
...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    def initialize_kv_cache(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        configured_cudagraph_mode = self.compilation_config.cudagraph_mode
        selected_cudagraph_mode = _r38_rank_local_graph_mode(
            configured_cudagraph_mode,
            self.pp_rank,
            self.parallel_config.pipeline_parallel_size,
        )
        self.compilation_config.cudagraph_mode = selected_cudagraph_mode
        ...
        if os.getenv("R38_PP1_FULL_DECODE", "0") == "1":
            if cudagraph_mode != selected_cudagraph_mode:
                raise RuntimeError(
                    "R38 rank-local graph mode was downgraded: "
                    f"selected={selected_cudagraph_mode.name}, "
                    f"resolved={cudagraph_mode.name}"
                )
            logger.warning(
                "R38_GRAPH_CONFIG pp_rank=%d configured=%s selected=%s "
                "resolved=%s capture_sizes=%s",
                self.pp_rank,
                configured_cudagraph_mode.name,
                selected_cudagraph_mode.name,
                cudagraph_mode.name,
        ...
        if os.getenv("R38_PP1_FULL_DECODE", "0") == "1":
            planned = {
                ...
                for mode, descs in self.cudagraph_manager._capture_descs.items()
            }
            logger.warning("R38_GRAPH_CAPTURE_PLAN pp_rank=%d planned=%s", self.pp_rank, planned)
            ...
            if capture_decoder:
                ...
                if os.getenv("R38_PP1_FULL_DECODE", "0") == "1":
                    full_captured = [
                        ...
                        for desc in self.cudagraph_manager.graphs
                    ]
                    logger.warning(
                        "R38_GRAPH_CAPTURE_DONE pp_rank=%d full_captured=%s",
                        self.pp_rank,
                        full_captured,
                    )
                if self.speculator is not None:
    ...
    def update_pp_decode_requests(self):
        ...
        if self.pp_handler is not None:
            ...
            if outputs is not None:
                # Restore proposed drafts before the next verification input is
                # reconstructed on this non-last pipeline rank.
    ...
    def execute_model(
        ...
        gdn_graph_runtime.runner_tick(dummy_run)
        if not dummy_run:
        ...
        # Experiment-only: verify actual single-token dispatch; write once per rank.
        if not dummy_run and input_batch.num_tokens == 1:
            expected = os.environ.get("Q38_SCREEN_EXPECTED_MODE")
            if expected:
                assert batch_desc.cg_mode.name == expected, ("Q38 graph mode mismatch", self.pp_rank, batch_desc.cg_mode.name, expected)
                if not getattr(self, "_q38_screen_mode_logged", False):
                    logger.warning("Q38_SCREEN_MODE pp_rank=%d mode=%s", self.pp_rank, batch_desc.cg_mode.name)
                    self._q38_screen_mode_logged = True
        ...
        if (os.getenv("R38_PP1_FULL_DECODE", "0") == "1" and os.getenv("R38_TOKEN_DIAGNOSTICS", "0") == "1" and not dummy_run):
            self._r38_graph_step += 1
            intermediate_ptrs: dict[str, int] = {}
            graph_intermediate = model_inputs.get("intermediate_tensors")
            if isinstance(graph_intermediate, IntermediateTensors):
                intermediate_ptrs = {
                    name: tensor.data_ptr()
                    for name, tensor in graph_intermediate.tensors.items()
                }
            logger.warning(
                "R38_GRAPH_DISPATCH pp_rank=%d step=%d mode=%s reqs=%d "
                "padded_reqs=%d tokens=%d padded_tokens=%d req_slots=%s "
                "input_ptrs=%s intermediate_ptrs=%s block_table_ptrs=%s",
                self.pp_rank,
                self._r38_graph_step,
                ...
                {
                    "input_ids": self.input_buffers.input_ids.data_ptr(),
                    "positions": self.input_buffers.positions.data_ptr(),
                    "query_start_loc": self.input_buffers.query_start_loc.data_ptr(),
                    "seq_lens": self.input_buffers.seq_lens.data_ptr(),
                },
                intermediate_ptrs,
...
# --- bounded GDS PLE runner hook (VLLM_PLE_GDS_RUNNER=1) ---
from vllm_ple_gds_runner import install_runner_hooks as _install_gds_runner_hooks
_install_gds_runner_hooks()
