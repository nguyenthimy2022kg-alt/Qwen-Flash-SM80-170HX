...
class PendingRecv:
    ...
    # Proposed MTP draft tokens relayed from the last pipeline rank.
...
class PPHandler:
    ...
    def receive(self, input_batch: InputBatch) -> bool:
        ...
        with torch.cuda.stream(self.broadcast_stream):
            ...
            # MTP proposal runs only on the last rank. Relay its fixed-width
            # draft grid as the third ordered collective on this sibling group.
            draft_tokens = None
    ...
    def broadcast(
        ...
        # receive() always posts [num_reqs, num_speculative_steps + 1].
        # Prefill/first decode naturally produces width 1; NCCL does not
        # negotiate counts, so pad every send to the receiver's fixed width.
    ...
    def broadcast_draft(
        ...
        """Relay last-rank MTP proposals in the same per-step collective order."""
        assert self.is_last_rank
        if self.max_sample_len <= 1:
            return
        if compute_need_sampled_mask(input_batch) is None:
