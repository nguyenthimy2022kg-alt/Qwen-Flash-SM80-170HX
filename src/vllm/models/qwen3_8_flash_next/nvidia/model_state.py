...
class Qwen3_8FlashNextModelState(MambaHybridModelState):
    ...
    def __init__(
        ...
        # Raw token IDs are needed only on a stage that owns a PLE layer.
        # Qwen3.8-Flash-Next places its PLE at layer 1, which is on PP rank 0
        # for the supported PP=2 layout. Later ranks consume the hidden state
        # after PLE injection and must not prepare n-gram inputs themselves.
        pp_group = get_pp_group()
        self.uses_ngram_embedding = bool(config.ple_layer_ids) and pp_group.is_first_rank
